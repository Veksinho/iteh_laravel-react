...
export default class DodajModel extends React.Component {
    ...
    handleSubmit(event) {
      ...
      const modelNovi={
         tekst:this.state.model,proizvodjac_id:this.state.proizvodjacJedan,opis:this.state.opis
      }

      axios.post('http://127.0.0.1:8000/modeli/sacuvaj',modelNovi)
      .then(res=>{
        alert('Uspesno ste uneli novi model');
      });
